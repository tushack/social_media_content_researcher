  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("error", (error) => {
      if (error.code === "ENOENT") {
        return reject(
          createHttpError(
            "FFmpeg was not found. Install FFmpeg and set FFMPEG_PATH in backend/.env.",
            500
          )
        );
      }
      ...
      return reject(
        createHttpError(`FFmpeg could not start: ${error.message}`, 500)
      );
    });
    ...
    child.on("close", (code) => {
      ...
      return reject(
        createHttpError(
          detail
            ? `FFmpeg could not convert this file. ${detail}`
            : "FFmpeg could not convert this file. Please try another original video file.",
          422
        )
      );
    });
...
function getYoutubeMaxFileSizeMb() {
  ...
  return Math.max(50, Math.min(Math.floor(configured), 10_240));
}
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("error", (error) => {
      if (error.code === "ENOENT") {
        return reject(
          createHttpError(
            "yt-dlp was not found. Install yt-dlp and set YT_DLP_PATH in backend/.env.",
            500
          )
        );
      }
      ...
      return reject(
        createHttpError(`yt-dlp could not start: ${error.message}`, 500)
      );
    });
    ...
    child.on("close", (code) => {
      ...
      return reject(
        createHttpError(
          detail
            ? `Could not prepare this YouTube video. ${detail}`
            : "Could not prepare this YouTube video. Check the link and try again.",
          422
        )
      );
    });
...
async function downloadYoutubeSource({
  ...
  const tempPrefix = `youtube-${exportId}-`;
...
async function convertOwnedMedia({
  ...
  try {
    ...
    const fileStem = makeFileStem(
      youtubeTitle ||
        (resolvedYoutubeVideoId ? `youtube-${resolvedYoutubeVideoId}` : path.parse(sourceOriginalName).name)
    );
    ...
    if (error) {
      ...
      throw createHttpError(error.message || "Could not save media export history.", 500);
    }
...
async function getOwnedMediaExport({ userId, exportId }) {
  ...
  if (new Date(data.expires_at).getTime() <= Date.now()) {
    throw createHttpError("This export has expired. Create it again from your original file.", 410);
  }
...
async function purgeExpiredMediaExports() {
  ...
  if (error) {
    throw createHttpError(error.message || "Could not find expired media exports.", 500);
  }
