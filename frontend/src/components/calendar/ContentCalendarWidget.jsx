  ...
  events.forEach((event) => {
    ...
    lines.push(
      `DESCRIPTION:${escapeIcsText(
        `Platform: ${event.platform || "YouTube"} | Status: ${
          event.status || "Ideas"
        }`
      )}`
    );
  ...
  const handleDelete = async (id) => {
    const confirmed = window.appConfirm
      ? await window.appConfirm({
          ...
          message: "This content plan will be removed from your calendar.",
          confirmText: "Yes, Delete",
          cancelText: "Cancel",
        })
      : window.confirm("Delete this calendar item?");
                          ...
                          {monthDays.map((date, index) => {
                            ...
                            return (
                              <div
                                ...
                                className={`min-h-[115px] rounded-2xl border p-2 transition ${
                                  dateKey === todayKey
                      ...
                      {weekDays.map((date) => {
                        ...
                        return (
                          <div
                            ...
                            onDragOver={(event) => event.preventDefault()}
                            onDrop={() => handleDropOnDate(dateKey)}
                            className={`min-h-[320px] rounded-3xl border p-3 transition ${
                              dateKey === todayKey
                                ? "border-cyan-300/40 bg-cyan-300/10"
                                : "border-white/10 bg-white/[0.035]"
                            }`}
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex flex-col gap-3 pt-2 sm:flex-row">
                        <button
                          type="button"
                          onClick={handleSave}
                          className="h-12 flex-1 rounded-2xl bg-cyan-300 px-5 text-sm font-semibold text-black transition hover:bg-cyan-200"
                        >
                          Save Plan
                        </button>
                        ...
                        {editingId && (
                          <button
                            ...
                            className="h-12 rounded-2xl border border-red-400/20 bg-red-500/10 px-5 text-sm font-semibold text-red-200 transition hover:bg-red-500/20"
                          >
