  ...
  React.useEffect(() => {
    ...
    async function loadCalendar() {
      ...
      try {
        ...
        // One-time migration for plans saved in the old localStorage calendar.
          ...
          for (const localEvent of localEvents) {
            try {
              const created = await createCalendarEvent({
                ...
                notified: localEvent.notified,
              });

              if (created?.item) {
                migratedEvents.push(created.item);
              }
        ...
        if (active) {
          // Local data stays as a fallback if the backend is temporarily unavailable.
          setEvents(localEvents);
          setCalendarError(
            error.message ||
        ...
        <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/75 p-3 backdrop-blur-md sm:p-6">
          <div className="relative flex max-h-[92vh] w-full max-w-6xl flex-col overflow-hidden rounded-[2rem] border border-white/10 bg-[#080a12] shadow-2xl shadow-black">
            ...
            {(calendarLoading || calendarError) && (
              <div
                className={`mx-4 mt-4 rounded-2xl border px-4 py-3 text-sm sm:mx-5 ${
                  calendarError
