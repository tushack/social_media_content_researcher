...
export default function DataPrivacy() {
  ...
  const handleDeleteOneRecord = async (key) => {
    const confirmDelete = window.confirm(
      "This selected record type will be removed from your active workspace. Permanent deletion will be processed after 30 days. Continue?"
    );

    if (!confirmDelete) return;

    await handleDeleteRecords([key], true);
  };
  ...
  const handleDeleteRecords = async (
    ...
    if (!skipConfirm) {
      const confirmDelete = window.confirm(
        "Selected records will be removed from your active workspace. Permanent deletion will be processed after 30 days. Continue?"
      );
  ...
  const handleDeleteAccount = async () => {
    if (otp.length !== 4) {
      setError("Please enter valid 4-digit verification code.");
      return;
    }

    const confirmDelete = window.confirm(
      "This will delete your account access and remove saved ideas, research history, thumbnails, and profile from your active workspace. Permanent deletion will be processed after 300 days. Continue?"
    );
    ...
    try {
      ...
      const result = await confirmDeleteAccount(otp);

      await signOut(auth);

      alert(
        result.message ||
        "Account deleted from active access. Permanent deletion is scheduled after 300 days."
      );
          ...
          <CardContent className="p-5 sm:p-6">
            ...
            <div className="mt-5 grid gap-4 rounded-3xl border border-white/10 bg-black/20 p-4">
              <div>
                ...
                <p className="mt-1 text-sm leading-6 text-zinc-500">
                  We will send a 4-digit code to your registered email before
                  account deletion.
                </p>
              </div>
              ...
              {otpSent && (
                <div className="grid gap-4">
                  ...
                  <Button
                    ...
                    disabled={deletingAccount || otp.length !== 4}
